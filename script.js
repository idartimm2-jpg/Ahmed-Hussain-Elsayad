...
// ⚠️ استبدل هذه القيم ببيانات حسابك من EmailJS
emailjs.init("YOUR_PUBLIC_KEY"); // من https://dashboard.emailjs.com/admin/account

const EMAIL_SERVICE_ID = "YOUR_SERVICE_ID";
const EMAIL_TEMPLATE_ID = "complaint_template";
const RECIPIENT_EMAIL = "your-email@gmail.com"; // ضع بريدك الذي تريد الاستقبال عليه
...
function setupFormSubmit() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            // Upload files and audio to Firebase
...
async function uploadAttachments() {
    ...
    // تحويل الملفات إلى Base64 للإرسال عبر البريد
    ...
    // انتظر قليلاً لإكمال تحويل الملفات
...
function formatWhatsAppMessage(data) {
    ...
    if (data.attachmentURLs && data.attachmentURLs.length > 0) {
        attachmentText = '\n\n📎 *المرفقات:*\n';
        data.attachmentURLs.forEach((attachment, index) => {
            attachmentText += `${index + 1}. ${attachment.name}: ${attachment.url}\n`;
        });
    }
...
⏰ *التاريخ والوقت:* ${data.timestamp}${attachmentText}
    `.trim();
...
async function sendComplaintEmail() {
    try {
        const templateParams = {
            ...
            complaint: document.getElementById('complaint').value,
            timestamp: new Date().toLocaleString('ar-EG'),
            attachments: uploadedFileURLs.length > 0 ? 
                `تم إرسال ${uploadedFileURLs.length} ملف(ات)` : 'لا توجد مرفقات'
        };
